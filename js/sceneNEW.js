...
let buttonBackAnimation = document.getElementById("button-back-animation");
let buttonBackScene = document.getElementById("button-back-scene");
let buttonOffMotor = document.getElementById("button-off-motor");
let buttonBlack = document.getElementById("button-black");
...
const file = '../audio/clickMouse.wav'; 
...
function OnClickButton() {
    ...
    if(intersects.length > 0){
        ...
        if(i.object.name == "buttonPower") {
            if(audio.isPlaying){   
                audio.stop();
                buttonBackScene.style.display= "block";
            } else {
                ...
                audio.setBuffer(buffer);
                audio.play();
                buttonBackScene.style.display= "none";
                })
            }
        };
        });
...
function camAnimation(xTarget,yTarget,zTarget,tweenDuration, xCam, yCam, zCam){
    ...
    targetTween.onComplete(function() {
      //  audio.stop();
        ...
        if(animationIndex >= 2 ){
            buttonBackAnimation.style.display= "block";
            CanvasDescription();
        }
...
function CanvasDescription() {
    buttonBackAnimation.addEventListener("click", function(){
        buttonBackAnimation.style.display="none";
        animationIndex = 0;
...
function BackMainScene() {
    buttonBackScene.addEventListener("click", function(){
        isAnimation = true;
        animationIndex = 0;
        buttonBackScene.style.display="none";
        camAnimation(0, 0, 0, 2800, 3.27, 0.31, 0.09);
...
camera.rotation.x -= window.innerHeight;
camera.rotation.y -= window.innerWidth * 0.75;
}
